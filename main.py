...
from src.evaluation import compute_reconstruction_error,plot_training_history, plot_error_distribution,predict_anomalies,evaluate_model,plot_confusion_matrix,plot_roc_curve,plot_pr_curve,threshold_analysis, save_metrics, calculate_threshold, attack_type_analysis
...
train_reconstructions = autoencoder.predict(X_training_set)
train_mse = np.mean(np.square(X_training_set - train_reconstructions),axis=1)
plot_training_history(history)
reconstructions, mse = compute_reconstruction_error(autoencoder, X_test)
threshold = calculate_threshold(train_mse)
print("Calculated Threshold:", threshold)
plot_error_distribution(mse,y_test)
y_pred = predict_anomalies(mse,threshold)
evaluate_model(y_test, y_pred, mse)
plot_confusion_matrix(y_test,y_pred)
plot_roc_curve(y_test, mse)
plot_pr_curve(y_test,mse)
threshold_analysis(mse,y_test)
attack_type_analysis(test_df,mse)
